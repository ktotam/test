...
public class Service {
    ...
    private Double newDps(Form form, double as, double dmg) {
        ...
        if (damage < 1)
            damage = 1.0;
        if (!form.getBaseDmg().equals(1.47)) {
            Double temp = (form.getBaseDmg() * damage * (1 + form.getAuraDmg())) / (1 / (0.5 * (Math.pow(form.getBonusAs() + 1, as)))) +
                    form.getBaseDmg() * form.getAllDmgUpgrades() / (1 / (0.5 * (Math.pow(form.getBonusAs() + 1, as))));
            if (temp < 0)
                return 1.0;
            return temp;
        }
        Double temp = (form.getBaseDmg() * damage * (1 + form.getAuraDmg())) / (1 / (0.25 * (Math.pow(form.getBonusAs() + 1, as)))) +
